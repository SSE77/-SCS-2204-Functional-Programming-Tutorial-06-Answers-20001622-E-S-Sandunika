object caeserCipher
{
    val alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ" // define alphabet 

    def encryption():Any=
    {
        val sendmassage = scala.io.StdIn.readLine("The massage needed to be sent: ") // read secret message
        val shift = (scala.io.StdIn.readLine("Shift from: ").toInt + alphabet.size) % alphabet.size // characters to shift
    ...
    def decryption():Any={
        ...
        val decryptedmassage = send2massage.map((c:Char) => 
        {
            val x = alphabet.indexOf(c.toUpper)
